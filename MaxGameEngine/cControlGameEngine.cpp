...
glm::quat cControlGameEngine::GetQuatRotation(std::string modelName)
{
    cMesh* rotatedMesh = g_pFindMeshByFriendlyName(modelName);

    return rotatedMesh->get_qOrientation();
}
...
bool cControlGameEngine::ComparePhysicsObjectWithAABBs(std::string modelName)
{
    ...
    {
        if (boundingBox->IsInsideAABB(meshObject->drawPosition))
        {
...
void cControlGameEngine::AddTrianglesToAABB(std::string modelName)
{
    ...
    //----------------------------Delete AABB meshes that dont have any triangles in them-----------------------------------

    for (auto iteratorAABB = AABBList.begin(); iteratorAABB != AABBList.end();)
    {
        if ((*iteratorAABB)->trianglesInsideAABBList.empty())
        {
            DeleteMesh((*iteratorAABB)->meshModelName);

            delete* iteratorAABB;
            iteratorAABB = AABBList.erase(iteratorAABB);
        }
        else
            ++iteratorAABB;
    }

    //-------------------------------------Printing those with triangles in them--------------------------------------------
...
void cControlGameEngine::MakeAllAABBsInvisible()
{
    for (cAABB* AABBMesh : AABBList)
    {
        TurnVisibilityOn(AABBMesh->meshModelName);
        TurnMeshLightsOn(AABBMesh->meshModelName);
    }
}


bool cControlGameEngine::IsWithinBoundsOfModelAABB(std::string modelName)
{
    cMesh* meshObject = g_pFindMeshByFriendlyName(modelName);

    for (cAABB* AABBMesh : AABBList)
    {
        AABBMesh->IsInsideAABB(meshObject->drawPosition);
        return true;
    }
