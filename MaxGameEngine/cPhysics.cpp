...
bool cPhysics::CheckForAABBTriangleCollision(std::vector<sTriangle> triangleList, sPhysicsProperties* spherePhysicalProps)
{
...
void cPhysics::EulerForwardIntegration(sPhysicsProperties* physicsModel, double deltaTime)
{
	...
	glm::vec3 velocityChange = physicsModel->sphereProps->acceleration * (float)deltaTime * 1.5f;

	//physicsModel->sphereProps->velocity *= dampingFactor;

	physicsModel->sphereProps->velocity += velocityChange;

	glm::vec3 positionChange = physicsModel->sphereProps->velocity * (float)deltaTime * 1.5f;
